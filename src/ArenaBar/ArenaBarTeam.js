...
export default class ArenaBarTeam extends Component {
  ...
  renderName = (name, i) => {
    ...
    return usernames.map((name, i) => {
      ...
      return [
        <span className="arena-bar-team__name--item">{name}</span>,
        <span className="arena-bar-team__name--seperator">/</span>
      ];
    });
  };
    ...
    return (
      <div className="arena-bar-team">
        ...
        <div className={avatarClasses}>
          {avatars &&
            avatars.map(avatar => <img src={avatar} className="arena-bar-team__avatar--img" />)}
        </div>
